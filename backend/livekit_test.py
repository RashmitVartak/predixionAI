...
from livekit.agents import AgentSession, Agent, WorkerOptions
from livekit.agents import function_tool, get_job_context, RunContext, ChatContext, JobContext
from livekit.plugins import (
    groq,
    elevenlabs,
    deepgram,
...
# WebSocket client to communicate with FastAPI server
SOCKET_URL = "wss://3bs69l0z-5000.inc1.devtunnels.ms/ws"
...
class VoiceAgent(Agent):
    ...
    async def hangup(self):
        ...
        # Send call completed status
        await send_websocket_message({
            "event": "call_status",
            "data": {"phone": json.loads(job_ctx.job.metadata).get("phone"), "status": "Completed"}
        })
...
async def entrypoint(ctx: JobContext):
    ...
    initial_ctx.add_message(
        ...
        content=f"""
            You are talking to our customer named {first_name} {last_name}.
            They have a total outstanding loan repayment balance of Rupees {balance_to_pay}.
            According to their agreement they need to pay Rupees {installment} as monthly installment.
        """
    )
    ...
    # Send call initiated status
    await send_websocket_message({
        "event": "call_status",
        "data": {"phone": phone, "status": "Initiated"}
    })

    async def monitor_transcriptions():
        """Monitor chat context for new messages to capture transcriptions"""
        while True:
            for message in initial_ctx.messages:
                if message.role == "user" and message.content:
                    ...
                    # Clear the message to avoid re-sending
                    message.content = ""
                    ...
                    logger.info(f"Riya transcript: {message.content}")
                    message.content = ""
            await asyncio.sleep(0.1)  # Avoid tight loop

    # Start transcription monitoring in the background
    transcription_task = asyncio.create_task(monitor_transcriptions())
    ...
    try:
        ...
        # Send call in progress status
        await send_websocket_message({
            "event": "call_status",
            "data": {"phone": phone, "status": "In Progress"}
        })
        await session_started
        participant = await ctx.wait_for_participant(identity=customer)
        logger.info(f"participant joined: {participant.identity}")
        await session.generate_reply(instructions=f"Greet the user with their name: {customer}.")
    except api.TwirpError as e:
        logger.error(
            f"error creating SIP participant: {e.message}, "
            f"SIP status: {e.metadata.get('sip_status_code')} "
        ...
        await send_websocket_message({
            "event": "call_status",
            "data": {"phone": phone, "status": f"Failed: {e.message}"}
        })
        ctx.shutdown()
    finally:
